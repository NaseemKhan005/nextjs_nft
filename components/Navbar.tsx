...
const Navbar = () => {
	...
	return (
		<div className="relative w-full py-2 backdrop-blur-xl bg-purple/10">
			<div className="flex items-center justify-between container px-5 md:px-10 mx-auto">
				...
				<div className="relative">
					<ul
						className={cn(
							showNavbar ? "right-0 top-0" : "-right-full -top-full",
							"flex md:items-center gap-4 xl:gap-8 md:relative fixed md:right-0 md:top-0 w-80 h-80 px-8 md:px-0 py-14 md:py-0 rounded-bl-full md:w-auto md:h-auto backdrop-blur-xl bg-purple dark:bg-white md:bg-transparent dark:md:bg-transparent md:rounded-none md:backdrop-blur-none dark:md:backdrop-blur-none flex-col md:flex-row z-[100] transition-all duration-500"
						)}
					>
						{navLinks.map((link) => (
							<li key={link.link} className="text-end nav-links">
								<Link
									href={link.link}
									className={cn(
										pathname === link.link
